...
import '../styles/controls/pagination.scss';
...
class CustomPagination extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.currentPage !== nextProps.currentPage ||
      ...
      this.props.maxPageSize !== nextProps.maxPageSize) {
      const {
    ...
    if (maxPageSize < totalPage || mobile) {
      const prevElement = (
        <li key="prev" className={`page-item prev${currentPage <= 1 ? ' disabled' : ''}`}>
          <button className="page-link" onClick={() => onPageChange('prev')} tabIndex="-1">
    ...
    if (maxPageSize < totalPage || mobile) {
      const nextElement = (
        <li key="next" className={`page-item next${currentPage >= totalPage ? ' disabled' : ''}`}>
          <button className="page-link" onClick={() => onPageChange('next')}tabIndex="-1">
